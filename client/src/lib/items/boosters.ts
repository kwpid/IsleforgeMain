...
export const BOOSTER_ITEMS: BoosterDefinition[] = [
  {
    ...
    name: "Booster Cookie",
    description:
      "A magical cookie infused with mining essence. Grants temporary mining bonuses.",
    icon: "cookie.png",
    rarity: "rare",
    sellPrice: 23500,
    duration: 15 * 60,
    effects: [
      ...
      { stat: "mining_luck", multiplier: 0.1 },
  ...
  },
  {
    ...
    name: "Booster Carrot",
    description:
      "A magically enhanced carrot that accelerates crop growth. Especially effective for carrots!",
    icon: "carrot.png",
    rarity: "rare",
    sellPrice: 21000,
    duration: 30 * 60,
    effects: [
      ...
      { stat: "crop_speed_carrot", multiplier: 0.5 },
  ...
  },
  {
    id: "booster_potato",
    name: "Booster Potato",
    description: "A legendary farmer once held the #1 potato spot..",
    icon: "potato.png",
    rarity: "epic",
    sellPrice: 35000,
    duration: 30 * 60,
...
export function getBoosterDisplayStats(booster: BoosterDefinition): string[] {
  const displayLabels: Record<string, (mult: number) => string> = {
    ...
    crop_speed_all: (m) =>
      `+${(m * 100).toFixed(0)}% Faster Crop Grow Time (All)`,
    crop_speed_carrot: (m) =>
      `+${(m * 100).toFixed(0)}% Faster Carrot Grow Time`,
    crop_speed_potato: (m) =>
      `+${(m * 100).toFixed(0)}% Faster Potato Grow Time`,
    farming_xp: (m) => `+${(m * 100).toFixed(0)}% More XP from Farming`,
    farm_sale_profit: (m) =>
      `+${(m * 100).toFixed(0)}% More Profit from Farm Items`,
    potato_sale_profit: (m) =>
      `+${(m * 100).toFixed(0)}% More Profit from Potato Items`,
  };

  return booster.effects.map((effect) => {
    const formatter = displayLabels[effect.stat];
    return formatter
      ? formatter(effect.multiplier)
